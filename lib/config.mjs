...
const MCP_STDIO_TRANSPORT_FIELDS = ["command", "args", "env", "env_vars", "cwd"];
const MCP_HTTP_TRANSPORT_FIELDS = [
	"url", "bearer_token_env_var", "http_headers", "env_http_headers", "http_headers_helper",
	"auth", "oauth", "oauth_resource",
];
const MCP_UNSUPPORTED_FIELDS = ["bearer_token", "experimental_environment", "identity"];
...
function validateManagedMcpTransports(current, fragment) {
	const fragmentServers = fragment.mcp_servers;
	if (!fragmentServers || typeof fragmentServers !== "object" || Array.isArray(fragmentServers) || hasTomlLiteralMarker(fragmentServers)) return;
	const currentServers = current.mcp_servers;
	for (const [serverName, fragmentServer] of Object.entries(fragmentServers)) {
		if (!fragmentServer || typeof fragmentServer !== "object" || Array.isArray(fragmentServer) || hasTomlLiteralMarker(fragmentServer)) continue;
		const hasCommand = Object.hasOwn(fragmentServer, "command");
		const hasUrl = Object.hasOwn(fragmentServer, "url");
		if (!hasCommand && !hasUrl) continue;
		const incompatibleFields = [
			...(hasCommand ? MCP_HTTP_TRANSPORT_FIELDS : MCP_STDIO_TRANSPORT_FIELDS),
			...MCP_UNSUPPORTED_FIELDS,
		];
		const currentServer = currentServers && typeof currentServers === "object" && !Array.isArray(currentServers)
			? currentServers[serverName]
			: undefined;
		const sources = [fragmentServer];
		if (currentServer && typeof currentServer === "object" && !Array.isArray(currentServer) && !hasTomlLiteralMarker(currentServer)) {
			sources.push(currentServer);
		}
		const conflicts = incompatibleFields.filter((field) => sources.some((server) => Object.hasOwn(server, field)));
		if (conflicts.length) {
			throw new Error(`managed MCP transport conflicts at ${conflicts.map((field) => `mcp_servers.${serverName}.${field}`).join(", ")}`);
		}
	}
}
...
export function mergeManaged(current, fragment, { allowEqual = true, migrateConflicts = false } = {}) {
	...
	validateManagedMcpTransports(current, fragment);
	const result = clone(current);
	const paths = [];
