...
function validateState(state) {
	if (!state || typeof state !== "object" || !new Set([LEGACY_SCHEMA, STATE_SCHEMA]).has(state.schema) || typeof state.version !== "string" || !Array.isArray(state.resources)) {
...
function validateTransactionResourceSet(transaction, current) {
	...
	const addState = (state) => {
		for (const resource of state?.resources ?? []) expected.add(resourceKey(resource));
	};
...
export class Lifecycle {
	...
	async captureExpectedState(expectedState) {
		...
		try {
			...
			throw new Error(`managed state is invalid JSON: ${error.message}`);
		}
	...
	async prepareResource(artifact, previous, { adopt = false, migrateCodexLauncher = false } = {}) {
	...
	async rollbackLocked(options = {}) {
		...
		if (!transaction || !new Set([LEGACY_SCHEMA, TRANSACTION_SCHEMA]).has(transaction.schema) || transaction.id !== id || !Array.isArray(transaction.resources)) {
			...
			if (record.kind === "file") {
				...
				if (transaction.schema === LEGACY_SCHEMA) {
					...
					if (["beforeSha256", "afterSha256", "beforeMode", "afterMode"].some((field) => Object.hasOwn(record, field))) {
						throw new Error("schema-4 transaction contains legacy file metadata");
					}
