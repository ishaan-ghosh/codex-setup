	...
	if (resource.kind === "file") {
		...
		if (schema === STATE_SCHEMA) {
			if (!hasDisplacedValues || !Array.isArray(resource.displacedValues)) {
				throw new Error("schema-5 managed state is missing displaced config metadata");
			}
...
function validateState(state) {
	if (!state || typeof state !== "object" || !new Set([LEGACY_SCHEMA, SNAPSHOT_SCHEMA, STATE_SCHEMA]).has(state.schema) || typeof state.version !== "string" || !Array.isArray(state.resources)) {
...
function validateTransactionResourceSet(transaction, current) {
	...
	const addState = (state) => {
		for (const resource of state?.resources ?? []) {
			const key = resourceKey(resource);
			if (expected.has(key) && expected.get(key) !== resource.kind) {
				throw new Error("rollback transaction state resources disagree on kind");
			}
			expected.set(key, resource.kind);
		}
	};
	if (transaction.action === "install" || transaction.action === "adopt") addState(current);
	else if (transaction.action === "uninstall") addState(transaction.previousState);
	else {
		addState(current);
		addState(transaction.previousState);
	}
	...
	for (const record of transaction.resources) {
		...
		actual.add(key);
		if (expected.has(key) && expected.get(key) !== record.kind) {
			throw new Error("rollback transaction resource kind does not match its states");
		}
	}
...
function validateMergeRollbackMetadata(transaction, current, record, key) {
	...
	if (transaction.schema === TRANSACTION_SCHEMA) {
		if (!Object.hasOwn(record, "displacedValues") || !Array.isArray(record.displacedValues)) {
			throw new Error("schema-5 rollback transaction is missing displaced config metadata");
		}
...
export class Lifecycle {
	...
	async prepareResource(artifact, previous, { adopt = false, migrateCodexLauncher = false, migrateManagedConfig = false } = {}) {
		...
		let base = previous?.managedPaths ? removeManaged(current, previous.managedPaths) : current;
		let merged;
		let displacedValues = [];
		...
		if (previous) {
			...
			base = restoreDisplacedValues(base, removedDisplaced, previous.managedPaths);
		...
		return {
			...
			managedDelta: createManagedRollbackDelta(current, merged.value, previous?.managedPaths ?? [], merged.paths, { displacedValues: rollbackDisplacedValues }),
			displacedValues,
	...
	async prepareRelease(release, state, options = {}) {
		...
		for (const old of previousByTarget.values()) {
			...
			if (old.kind === "file") {
				...
				let stripped = removeManaged(current, old.managedPaths ?? []);
	...
	async validatePreviousStateLineage(transaction) {
		...
		if (!predecessor || !new Set([LEGACY_SCHEMA, SNAPSHOT_SCHEMA, TRANSACTION_SCHEMA]).has(predecessor.schema)
	...
	async rollbackLocked(options = {}) {
		...
		if (!transaction || !new Set([LEGACY_SCHEMA, SNAPSHOT_SCHEMA, TRANSACTION_SCHEMA]).has(transaction.schema) || transaction.id !== id || !Array.isArray(transaction.resources)) {
	...
	async uninstallLocked() {
		...
		for (const resource of current.resources) {
			...
			if (resource.kind === "file") {
				...
				let stripped = removeManaged(value, resource.managedPaths ?? []);
